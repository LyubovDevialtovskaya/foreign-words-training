...
function updateWordsProgress() {
    const viewedPercent = Math.floor((currentIndex + 1) / arr.length * 100);
    wordsProgress.textContent = `${viewedPercent}%`;
}

function updateCorrectPercent() {
    const correctPercentValue = Math.floor((currentIndex + 1) / arr.length * 100);
    correctPercent.textContent = `${correctPercentValue}%`;
}

function displayWordStats() {
    const word = arr[currentIndex];
    const wordStats = wordStatsTemplate.content.cloneNode(true);
    wordStats.querySelector('.word-title').textContent = word.title;
    wordStats.querySelector('.word-translation').textContent = word.translation;
    wordStats.querySelector('.word-example').textContent = word.example;
...
window.addEventListener('load', () => {
    loadProgress();
    if (window.location.hash === '#exam-mode') {
        startTimer();
    }
});
